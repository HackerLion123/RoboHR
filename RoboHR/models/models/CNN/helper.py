...
def image_agumenter(train,test,shape):
	...
	train_set = datagen.flow_from_directory(train,subset="training",target_size=shape,batch_size=128,class_mode='categorical',shuffle=True)
	validation_set = datagen.flow_from_directory(train,subset="validation",target_size=shape,batch_size=128,class_mode='categorical',shuffle=True)


	#validate = datagen.flow_from_directory(validate,target_size=shape,batch_size=1,class_mode='categorical',shuffle=True)

	test_set = test_gen.flow_from_directory(test,target_size=shape,batch_size=1,class_mode=None,shuffle=False)
...
if __name__ == '__main__':
	t,v,l = image_agumenter("data/train","data/test",(64,64))
	print(t.samples)
